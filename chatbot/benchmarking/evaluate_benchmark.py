...
Combines:
1. Regex matching for exact factual data (phone numbers, addresses)
2. Cross-Encoder for semantic similarity (understands meaning)
3. NLI (Natural Language Inference) for information correctness
"""
...
# Configuration (Arabic defaults)
LANGUAGE = (os.getenv("BENCHMARK_LANGUAGE") or "ar").strip().lower()
...
class BenchmarkEvaluator:
    ...
    def __init__(self, factual_data_path: str = "factual_data.json"):
        ...
        if LANGUAGE.startswith("ar"):
            self.nli_labels = ["صحيح", "غير صحيح", "جزئيًا صحيح"]
            self.nli_template = "هذه الإجابة {}."
        else:

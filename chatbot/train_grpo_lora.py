"""
Reward-based GRPO LoRA fine-tuning for Qwen3-4B on Arabic homeless assistance data.
"""
...
JUDGE_MODEL_DEFAULT = (os.getenv("JUDGE_MODEL") or "deepseek/deepseek-chat").strip()
...
def load_prompts(raw_path: Path) -> list[str]:
    ...
    for conv in raw_data:
        ...
        if first.get("role") != "homeless":
            continue
        prompt = (
            "أنت مساعد اجتماعي يساعد الأشخاص بدون مأوى. "
            "كن متعاطفًا وعمليًا ولا تختلق معلومات.\n\n"
            f"الشخص: {first.get('content', '')}\n\n"
            "المساعد:"
        )
...
def call_judge_model(prompt: str, response: str, judge_model: str) -> float:
    ...
    judge_prompt = (
        "أنت تقيم ردًا لمساعد اجتماعي يقدم دعمًا للمشردين.\n"
        "قيّم الرد من 0 إلى 10 حسب: التعاطف، العملية، السلامة، والوضوح.\n\n"
        f"رسالة الشخص:\n{prompt}\n\n"
        f"رد المساعد:\n{response}\n\n"
        "أجب برقم فقط من 0 إلى 10."
    )

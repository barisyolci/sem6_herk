"""
Arabic Homeless Assistance Dataset Generator (multi-turn, Groq)

- Each /start creates a new run with unique filenames:
  dataset_raw_<run_id>.json
  dataset_rl_training_<run_id>.jsonl
- Runs are tracked in runs_manifest.json
- /merge creates:
...
GROQ_MAX_COMPLETION_TOKENS = int(os.getenv("GROQ_MAX_COMPLETION_TOKENS") or "8192")
GROQ_TEMPERATURE = float(os.getenv("GROQ_TEMPERATURE") or "1")
...
# ---------------------------
# Scenario pools (Arabic)
# ---------------------------
MOODS = [
    "قلق", "محبط", "خائف", "غاضب", "متعب", "حزين", "مرتبك", "ممتن",
    "يائس", "متوتر", "منهك", "خجول", "متردد", "مصمم"
]

BACKGROUNDS = [
    "فقدت عملي مؤخرًا", "خرجت من المستشفى للتو", "تعرضت للعنف الأسري",
    "انقطعت علاقتي بعائلتي", "تركت السكن بسبب الديون", "أعيش في الشارع منذ أسابيع",
    "لدي مشاكل صحية مزمنة", "أعاني من قلق شديد", "أواجه صعوبات قانونية",
    "أنا جديد في المدينة ولا أعرف الأماكن", "خرجت من الاحتجاز مؤخرًا",
    "أتعافى من الإدمان"
]

NEEDS = [
    "مكان آمن للمبيت الليلة", "وجبة دافئة", "رعاية طبية",
    "دعم نفسي", "مساعدة مالية", "استشارة قانونية",
    "علاج من الإدمان", "مساعدة في الأوراق الثبوتية",
    "ملابس دافئة", "الاستحمام والنظافة", "هاتف للاتصال",
    "خدمة إنترنت", "تخزين الأغراض", "مساعدة في التسجيل بالخدمات"
]

QUESTION_TYPES = [
    "طلب مساعدة", "طلب معلومات", "شرح الوضع", "استفسار عن الخدمات",
    "سؤال طارئ", "متابعة سؤال"
]
...
TIME_OF_DAY = ["الصباح الباكر", "بعد الظهر", "المساء", "منتصف الليل"]
WEATHER = ["بارد", "ممطر", "حار", "عاصف", "لطيف"]
...
def build_pair_prompt(scenario: dict) -> list[dict]:
    system = (
        "أنت تُولّد بيانات تدريبية بالعربية لمساعد محادثة خاص بالتشرد والدعم الاجتماعي.\n"
        "المخرجات يجب أن تكون JSON فقط بدون أي نص إضافي.\n"
        "الشكل المطلوب بالضبط:\n"
        '{ "turns": [ {"role":"homeless","content":"..."}, {"role":"helper","content":"..."}, ... ] }\n\n'
        "قواعد مهمة:\n"
        "- العربية فقط. بدون أي تنبيه عن الذكاء الاصطناعي.\n"
        "- لا تكتب عناوين أو أرقام هواتف أو مواقع أو بريد إلكتروني.\n"
        "- لا تذكر تعليمات طريق أو اتجاهات دقيقة.\n"
        "- في حالة خطر مباشر أو حالة طبية طارئة: قل \"اتصل بخدمات الطوارئ فورًا.\" ثم أنهِ الحوار.\n\n"
        "متطلبات الحوار:\n"
        "- من 4 إلى 6 تبادلات إجمالًا.\n"
        "- التبادل الأول: الشخص يصف وضعه ويسأل.\n"
        "- التبادل الثاني: المساعد يتعاطف ويطرح 1 إلى 3 أسئلة فرز قصيرة.\n"
        "- التبادل الثالث: الشخص يجيب بإجابات قصيرة واقعية.\n"
        "- التبادل الرابع: المساعد يقدم خطة واضحة من 3 إلى 7 خطوات، مع سؤال متابعة واحد قصير.\n"
        "- يمكن ذكر جهات عامة مثل البلدية، مركز إيواء، مركز اجتماعي، طبيب، خدمات اجتماعية.\n"
    )

    user = (
        "السيناريو:\n"
        f"- المدينة: {scenario['location']}\n"
        f"- الوقت: {scenario['time_of_day']} ({scenario['season']}، {scenario['weather']})\n"
        f"- الحالة النفسية: {scenario['mood']}\n"
        f"- الخلفية: {scenario['background']}\n"
        f"- الحاجة الأساسية: {scenario['primary_need']}\n"
        f"- الحاجة الثانوية: {scenario['secondary_need']}\n"
        f"- نوع السؤال: {scenario['question_type']}\n\n"
        "أعد JSON فقط."
    )
...
def parse_pair(text: str, scenario: dict) -> List[Dict]:
    ...
    person_1 = (
        f"أنا في {scenario['location']} وأحتاج مساعدة بخصوص {scenario['primary_need']}. "
        "لا أعرف من أين أبدأ."
    )
    helper_1 = (
        "أفهم أنك تمر بوقت صعب. هل أنت في مكان آمن الآن؟ وهل تحتاج لمكان للمبيت الليلة؟ "
        "هل أنت وحدك أم مع أطفال؟"
    )
    person_2 = "أنا وحدي. أحتاج مكانًا الليلة ولا أملك الكثير من الأغراض."
    helper_2 = (
        "شكرًا لتوضيحك. هذه خطوات ممكنة الآن:\n"
        "- ابحث عن مركز إيواء أو مركز اجتماعي لاستقبال أولي.\n"
        "- اطلب مساعدة عاجلة لمبيت الليلة، ثم ناقش خيارات الأيام القادمة.\n"
        "- إذا كانت لديك مشكلة بالأوراق، اطلب توجيهًا من الخدمات الاجتماعية.\n"
        "- إذا كان هناك خطر فوري، اتصل بخدمات الطوارئ فورًا.\n"
        "ما هي أقرب منطقة لك الآن؟"
    )
...
def make_training_records(conversations: list[dict]) -> list[dict]:
    ...
    base_system = (
        "أنت مساعد اجتماعي محترم ومتعاون.\n"
        "تحدث بالعربية بوضوح وبأسلوب عملي ومتوازن.\n"
        "لا تخترع عناوين أو أرقام هواتف أو روابط.\n"
        "إذا لم تكن متأكدًا، اذكر ذلك واسأل سؤالًا محددًا.\n"
        "في خطر مباشر أو حالة طبية طارئة: قل \"اتصل بخدمات الطوارئ فورًا.\".\n"
    )
    ...
    for conv in conversations:
        ...
        for idx, turn in enumerate(turns):
            ...
            if role == "homeless":
                history_lines.append(f"الشخص: {content}")
                continue

            if role == "helper":
                instruction = base_system + "\nسياق المحادثة حتى الآن:\n" + "\n".join(history_lines) + "\n\nرد المساعد:"

                last_user = ""
                for j in range(len(history_lines) - 1, -1, -1):
                    if history_lines[j].startswith("الشخص: "):
                        last_user = history_lines[j].replace("الشخص: ", "", 1)
                        break
...
<head>
  ...
  <title>Arabic Dataset Generator</title>
  <style>
...
<body>
  <div class="card">
    <h2>Arabic dataset generator</h2>

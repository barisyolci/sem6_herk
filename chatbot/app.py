...
OPENROUTER_MODEL = (os.getenv("OPENROUTER_MODEL") or "meta-llama/llama-3.1-8b-instant").strip()
OPENROUTER_MAX_TOKENS = int(os.getenv("OPENROUTER_MAX_TOKENS") or "600")
OPENROUTER_TEMPERATURE = float(os.getenv("OPENROUTER_TEMPERATURE") or "0.4")
...
TARGET_CONVERSATIONS_DEFAULT = int(os.getenv("TARGET_CONVERSATIONS") or "50")
PARALLEL_WORKERS = max(1, min(8, int(os.getenv("PARALLEL_WORKERS") or "2")))
...
def record_usage(usage: dict) -> None:
    ...
    total_tokens = usage.get("total_tokens", 0) if usage else (prompt_tokens + completion_tokens)
    with USAGE_LOCK:
        REQUESTS_TOTAL += 1
...
def estimate_tokens(messages: list[dict]) -> int:
    ...
    return estimated_prompt_tokens + OPENROUTER_MAX_TOKENS
...
def call_openrouter(system: str, user: str, max_retries: int=3) -> str:
    if not OPENROUTER_KEY:
        raise ValueError("OPENROUTER_KEY is missing. Set it in your .env.")
    last_err = None
    for attempt in range(max_retries):
        try:
            ...
            headers = {
                ...
                "X-Title": "HomelessSupportGenerator"
            }
            ...
            record_usage(usage)
            return sanitize_for_training(result["choices"][0]["message"]["content"].strip())
...
def build_pair_prompt(scenario: dict) -> Tuple[str, str]:
    system = (
...
def generate_one(conv_id: int) -> dict:
    ...
    system, user = build_pair_prompt(scenario)
    text = call_openrouter(system=system, user=user)
...
def generation_worker(run_id: str):
    ...
    generation_queue.put({
        "type": "status",
        "message": f"Started run: {run_id} (model={OPENROUTER_MODEL}, target={TARGET_CONVERSATIONS}, workers={PARALLEL_WORKERS})"
    })
